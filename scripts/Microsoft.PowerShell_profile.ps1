...
Function port-reset{netsh interface portproxy reset}
Function port{Param([Parameter(Mandatory=$true)][string]$from,[Parameter(Mandatory=$true)][string]$to);netsh interface portproxy add v4tov4 listenport=$to connectport=$from connectaddress=127.0.0.1}
